...
export const isStringArray = (value) => isArray(value) && value.length > 0 && value.every((x) => isString(x));
...
const isValidationError = (response) => errorResponseCodes.validationError.indexOf(response.status) > -1;
...
const allAuthErrorsHandler = (errorResponse, formikBag) => {
    const { setStatus } = formikBag;
    const { errors } = errorResponse.response.data;
    if (errors) {
        const error = errors[0];
        setStatus({ message: error.message, color: 'error' });
    }
};

export { formErrorsHandler, allAuthErrorsHandler };
